...
The reliance on $\{-1, 1\}$ states in the standard formulations of perceptrons and MLCMs
...
take values in $\{0, 1\}$, where $0$ represents a silent neuron and $1$ an active one.\\
With this change, the dynamics of the MLCM described in equation
\ref{eq:si_update} are expressed as:
\begin{equation}
    s_i^l(t+1) = 
    \begin{cases}
        \boldsymbol{\theta}\left(\displaystyle\sum_{j \neq i} J_{ij}^l s_j^l(t) + 
        J_D\, s_i^l(t) + \lambda_{\mathrm{IN}}\, x_i + \lambda_R^l\, s_i^{l+1}(t) - \gamma
        \right) & \text{if } l = 1 \\[2ex]
        \boldsymbol{\theta}\left(\displaystyle\sum_{j \neq i} J_{ij}^l s_j^l(t) + 
        J_D\, s_i^l(t) + \lambda_L^l\, s_i^{l-1}(t) + \lambda_R^l\, s_i^{l+1}(t)  - \gamma
        \right) & \text{if } 1 < l < L \\[2ex]
        \boldsymbol{\theta}\left(\displaystyle\sum_{j \neq i} J_{ij}^l s_j^l(t) + 
        J_D\, s_i^l(t) + \lambda_L^l\, s_i^{l-1}(t) + \lambda_{\mathrm{OUT}} 
        \sum_{k=1}^{M} W_{ik}^{\mathrm{back}} y_k  - \gamma\right) & \text{if } l = L
    \end{cases}
    \label{eq:si_sparse_update}
\end{equation}
Where $\boldsymbol{\theta}$ is the heaviside function and $\gamma$ is a threshold function that 
regulates the activation of the states. \\
...
sparsity of the MLCM. The threshold is set to a constant value $\gamma^l$ across all 
neurons in a layer $l$.
